...
local function CreateESPFor(pl)
    ...
    local function DoCreate(char)
        ...
        pcall(function()
            ...
            -- ── NAME BILLBOARD (AlwaysOnTop=true) ──
            local bb=Instance.new("BillboardGui")
            bb.Name="_LHESP_BB"
            bb.Size=UDim2.new(0,130,0,52)
            bb.AlwaysOnTop=true          -- сквозь стены
            ...
            nameLbl.TextColor3=ESPSettings.nameColor
            ...
            distLbl.Visible=ESPSettings.showName
            distLbl.Parent=bb
            data.distLbl=distLbl

            -- ── BOX через BoxHandleAdornment на каждый BasePart ──
            -- Используем линии через Frame в BillboardGui — самый надёжный способ
            -- Делаем отдельный BB для бокса с рамкой
            local bbBox=Instance.new("BillboardGui")
            bbBox.Name="_LHESP_BOX"
            bbBox.Size=UDim2.new(0,60,0,90)  -- примерный размер персонажа
            bbBox.AlwaysOnTop=true
            bbBox.StudsOffset=Vector3.new(0,0,0)
            bbBox.LightInfluence=0
            bbBox.Parent=hrp

            -- рамка (4 линии через Frame)
            local function MakeLine(size,pos,col)
                local f=Instance.new("Frame")
                f.Size=size
                f.Position=pos
                f.BackgroundColor3=col or ESPSettings.boxColor
                f.BorderSizePixel=0
                f.Parent=bbBox
                return f
            end
            local th=UDim.new(0,2)
            -- top
            ...
            bbBox.Visible=ESPSettings.showBox
            data.bbBox=bbBox

            -- ── HIGHLIGHT (Glow) — AlwaysOnTop через DepthMode ──
            local hl=nil
            pcall(function()
                ...
                -- DepthMode.AlwaysOnTop = видно сквозь стены
...
local function StartESP()
    ...
    -- update loop: дистанция + цвета + visibility
            ...
            for pl,data in pairs(ESPData) do
                ...
                if data.nameLbl then
                    pcall(function()
                        ...
                        data.nameLbl.TextColor3=ESPSettings.nameColor
                    end)
                end
                if data.distLbl then
                    pcall(function()
                        data.distLbl.Text=dist.."m"
                        data.distLbl.Visible=ESPSettings.showName and visible
                    end)
                end
                ...
                if data.bbBox then
                    pcall(function()
                        ...
                        if data.boxR   then data.boxR.BackgroundColor3=col   end
                    end)
                end
...
local function BuildESPPanel(inner)
    ...
    MakeToggleRow(inner,"Показывать бокс",ESPSettings.showBox,function(v)
        ...
        for _,data in pairs(ESPData) do
            if data.bbBox then pcall(function()data.bbBox.Visible=v end) end
        end
    end)
