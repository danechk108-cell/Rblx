...
-- ══════════════════════════════════════════════
-- NOTIFY (определяем РАНО — нужна в KeyScreen)
-- ══════════════════════════════════════════════
local notifStack={}
...
local function ShowKeyScreen(onSuccess)
   ...
   local function ValidateKey(key)
    ...
    -- ✅ Паттерн принимает A-Z и 0-9 (как генерирует бот)
    local p5 = "[A-Z0-9][A-Z0-9][A-Z0-9][A-Z0-9][A-Z0-9]"
    local pattern = "^LX%-"..p5.."%-"..p5.."%-"..p5.."%-"..p5.."$"

    if #key ~= 27 or not key:match(pattern) then
        SetStatus("❌ Неверный формат ключа", C.Red)
        return
    end
    ...
    confirmBtn.MouseButton1Click:Connect(function()
        ValidateKey(input.Text)
    end)
    input.FocusLost:Connect(function(enter)
        if enter then ValidateKey(input.Text) end
    end)
...
local function CreateESPFor(pl)
    ...
    local function DoCreate(char)
        ...
        pcall(function()
            ...
            bbBox.Size=UDim2.new(0,60,0,90)
...
-- ══════════════════════════════════════════════
-- ✅ ЕДИНСТВЕННЫЙ ЗАПУСК: Preloader → Key → UI
-- ══════════════════════════════════════════════
