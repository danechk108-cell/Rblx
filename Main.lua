...
local function ShowKeyScreen(onSuccess)
    ...
    if not key:match("^LX%-") or #key ~= 26 then
        SetStatus("❌ Неверный формат (длина: " .. #key .. ")", C.Red)
        return
    end
    ...
    task.spawn(function()
        local url = API_URL .. "/validate?key=" .. game:GetService("HttpService"):UrlEncode(key)
        print("[KEY] Запрос: " .. url)

        local ok, result = pcall(function()
            -- ✅ Пробуем все способы HTTP запроса
            local response = nil

            -- Способ 1: request() — стандарт для Delta/Solara/Fluxus
            if request then
                local res = request({
                    Url = url,
                    Method = "GET"
            ...
            -- Способ 2: http_request() — старые экзплойты
            ...
            -- Способ 4: HttpService (если разрешён)
            elseif pcall(function()
                response = game:GetService("HttpService"):GetAsync(url, true)
            end) then
                -- response уже установлен

            else
                error("HTTP недоступен")
            end

            print("[KEY] Ответ: " .. tostring(response))
        ...
        if not ok then
            ...
            SetStatus("❌ HTTP заблокирован экзплойтом", C.Red)
            return
        end
