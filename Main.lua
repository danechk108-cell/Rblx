...
local C={
    BG0=Color3.fromRGB(20,21,24),
    BG1=Color3.fromRGB(26,27,31),
    BG2=Color3.fromRGB(33,34,39),
    BG3=Color3.fromRGB(40,42,48),
    BG4=Color3.fromRGB(48,50,57),
    BG5=Color3.fromRGB(56,58,66),
    Acc=Color3.fromRGB(130,132,142),
    TxW=Color3.fromRGB(228,228,224),
    TxH=Color3.fromRGB(175,175,170),
    TxM=Color3.fromRGB(115,116,112),
    TxL=Color3.fromRGB(72,74,70),
    TxF=Color3.fromRGB(44,46,42),
    Bord=Color3.fromRGB(48,50,58),
    BordH=Color3.fromRGB(68,70,80),
    BordA=Color3.fromRGB(95,97,108),
    Sep=Color3.fromRGB(36,38,44),
    Green=Color3.fromRGB(88,178,108),
    Red=Color3.fromRGB(188,72,72),
    White=Color3.fromRGB(255,255,255),
    SlT=Color3.fromRGB(22,23,28),
    Tog=Color3.fromRGB(95,97,110),
}
...
local function MakeSlider(parent,opts)
    ...
    -- wrapper растягивается по ширине родителя
    ...
    Rnd(cont,8)
    Pd(cont,nil,8,8,10,10)

    -- label row
    local lblRow=New("Frame",{Size=UDim2.new(1,0,0,16),BackgroundTransparency=1},cont)
    New("TextLabel",{Size=UDim2.new(0.65,0,1,0),Text=opts.label or "",TextSize=10,
    ...
    local function UpdateFromX(x)
        ...
        local pct=math.max(0,math.min(1,(x-ax)/aw))
        SetVal(mn+pct*(mx-mn))
    end

    -- invisible hit area over the whole cont
    local hit=New("TextButton",{
        Size=UDim2.new(1,0,0,40),Position=UDim2.new(0,0,0,16),
        BackgroundTransparency=1,Text="",ZIndex=10,BorderSizePixel=0
    },cont)
    if hit then
        hit.MouseButton1Down:Connect(function(x,_) dragging=true;UpdateFromX(x) end)
...
local function MakeCard(parent,item)
    local BASE_H=60
    local PARAM_H=72  -- высота слайдер-панели
    local GAP=4

    -- WRAP — снаружи, меняет размер когда открыт слайдер
    ...
    local card=New("Frame",{
        Size=UDim2.new(1,0,0,BASE_H),  -- фиксированная высота
    ...
    -- PARAM PANEL — ниже карточки
    local paramPanel=nil
    if item.param then
        paramPanel=New("Frame",{
            Size=UDim2.new(1,-4,0,0),  -- начинает с 0 высоты (скрыт)
            Position=UDim2.new(0,2,0,BASE_H+GAP),
            BackgroundColor3=C.BG1,
            BorderSizePixel=0,
            ClipsDescendants=true,  -- обрезает содержимое пока высота=0
            Visible=true,
            ZIndex=2,
        },wrap)
        if paramPanel then
            ...
            if inner then
                ...
                MakeSlider(inner,{
                    label=pp.label,
                    min=pp.min,max=pp.max,step=pp.step,val=pp.val,fmt=pp.fmt,
                    onChange=function(v)
                        pp.val=v
    ...
    local function ApplyVisual(s)
        ...
        -- show/hide param panel by changing height
        if paramPanel then
            pcall(function()
                if s then
                    -- открыть: wrap увеличивается, paramPanel показывает слайдер
                     ...
                     for _,pl in ipairs(Players:GetPlayers()) do
                         if pl~=LP and pl.Character then
                             ...
                             if oh then
                                 ...
                                 if d<dist then dist=d;near=oh end
                             end
                         end
                     end
                     if near then hrp.CFrame=near.CFrame*CFrame.new(0,0,3.5) end
                 end)
             end)
         end,
         disable=function(it)if it.conn then it.conn:Disconnect();it.conn=nil end end},
         ...
         enable=function(it)
             it.conn=LP.CharacterAdded:Connect(function(char)
                 ...
                 if hum then hum.Died:Connect(function()
                     task.wait(1);pcall(function()LP:LoadCharacter()end) end) end
             end)
         end,
         disable=function(it)if it.conn then it.conn:Disconnect();it.conn=nil end end},
    }},
         ...
         enable=function(it)
             ...
             local function MakeTag(pl)
                 ...
                 local function DoTag(char)
                     ...
                     pcall(function()
                         local hrp=char:FindFirstChild("HumanoidRootPart");if not hrp then return end
                         local o=hrp:FindFirstChild("_LHESP");if o then o:Destroy() end
                         local bb=Instance.new("BillboardGui")
                         bb.Name="_LHESP";bb.Size=UDim2.new(0,90,0,34)
                         bb.AlwaysOnTop=true;bb.StudsOffset=Vector3.new(0,3,0);bb.Parent=hrp
                         New("TextLabel",{Size=UDim2.new(1,0,0,18),Text=pl.Name,TextSize=11,
                             Font=Enum.Font.GothamBold,TextColor3=C.Red,BackgroundTransparency=1,
                             TextStrokeTransparency=0,TextStrokeColor3=Color3.new(0,0,0)},bb)
                         it._esp[pl.UserId]=bb end)
                 end
         ...
         disable=function(it)
             ...
             for _,pl in ipairs(Players:GetPlayers()) do
                 if pl~=LP and pl.Character then
                     ...
                     if hrp then local t=hrp:FindFirstChild("_LHESP");if t then t:Destroy() end end
                 end
             end
         end},
...
local OTHER_SECTIONS={
    ...
    {title="НАСТРОЙКИ МЕНЮ",items={
        ...
        {label="Mini Btn Size",icon="BTN",desc="Размер кнопки показа",state=false,
         param={label="Размер кнопки",min=30,max=80,step=2,val=46,
          fmt=function(v)return math.floor(v).."px"end,
...
New("Frame",{Size=UDim2.new(1,0,0,14),Position=UDim2.new(0,0,1,-14),
    BackgroundColor3=C.BG2,BorderSizePixel=0,ZIndex=5},hdr)
-- separator line
New("Frame",{Size=UDim2.new(1,0,0,1),Position=UDim2.new(0,0,1,-1),
    BackgroundColor3=C.Sep,BorderSizePixel=0,ZIndex=6},hdr)
...
-- Title
New("TextLabel",{Size=UDim2.new(0,160,0,18),Position=UDim2.new(0,50,0,7),
...
New("TextLabel",{Size=UDim2.new(0,160,0,13),Position=UDim2.new(0,51,0,27),
    Text="v26 Player Edition",TextSize=9,Font=Enum.Font.Gotham,TextColor3=C.TxL,
    BackgroundTransparency=1,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=6},hdr)

-- Active badge — по центру-правее
local activeBadge=New("TextLabel",{
    Size=UDim2.new(0,64,0,20),
    Position=UDim2.new(0,0.5,0.5,-10),  -- будет перепозиционирован
...
-- позиционируем вручную: правее названия, левее кнопок
activeBadge.Position=UDim2.new(0,220,0.5,-10)
...
-- Header buttons — крепим к правому краю хедера
-- Кнопка X справа на -10, кнопка - на -42
local function MkHBtn(rightOffset, txt, hc, cb)
    local b=New("TextButton",{
        Size=UDim2.new(0,26,0,26),
        Position=UDim2.new(1, rightOffset, 0.5, -13),
        BackgroundColor3=C.BG3,BorderSizePixel=0,
        Text=txt,TextSize=11,Font=Enum.Font.GothamBold,
        TextColor3=C.TxM,AutoButtonColor=false,ZIndex=7,
    },hdr)
    if not b then return end
    Rnd(b,7);Strk(b,C.Bord,1)
    b.MouseEnter:Connect(function()
        Tw(b,{BackgroundColor3=hc},0.1)
...
local tabBar=New("Frame",{
    ...
    BackgroundColor3=C.BG2,BorderSizePixel=0,ZIndex=4,
},clip)
New("Frame",{Size=UDim2.new(1,0,0,1),Position=UDim2.new(0,0,1,-1),
    BackgroundColor3=C.Sep,BorderSizePixel=0,ZIndex=5},tabBar)
...
New("Frame",{Size=UDim2.new(1,-20,0,1),Position=UDim2.new(0,10,0,0),
    BackgroundColor3=C.Sep,BorderSizePixel=0,ZIndex=5},ftr)
...
RefreshContent()
UpdateBadge()
Notify("LUXURY HUB v26","OK! "..#ALL_ITEMS.." функций",C.Acc)
print("LUXURY HUB v26 - OK | "..#ALL_ITEMS.." items")