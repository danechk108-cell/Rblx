...
local function ShowPreloader(onDone)
    ...
    task.delay(0.3,function()
        -- fade in letters one by one feel
...
-- ══════════════════════════════════════════════
-- INLINE TOGGLE ROW (for param panels)
-- ══════════════════════════════════════════════
...
local function MakeCard(parent,item)
    ...
    local card=New("Frame",{
        Size=UDim2.new(1,0,0,BASE_H),Position=UDim2.new(0,0,0,0),
        BackgroundColor3=item.state and C.BG4 or C.BG2,
        BorderSizePixel=0,ClipsDescendants=false,ZIndex=1,
    },wrap)
    Rnd(card,12)
    ...
    if item.param or item.buildParamPanel then
        paramPanel=New("Frame",{
            ...
            BorderSizePixel=0,ClipsDescendants=true,Visible=true,ZIndex=2,
        },wrap)
        if paramPanel then
            ...
            if inner then
                ...
                if item.buildParamPanel then
                    item.buildParamPanel(inner)
...
local function CreateESPFor(pl)
    ...
    local function DoCreate(char)
        ...
        pcall(function()
            ...
            local nameLbl=New("TextLabel",{
                ...
                TextColor3=ESPSettings.nameColor,BackgroundTransparency=1,
                TextXAlignment=Enum.TextXAlignment.Center,
                TextStrokeTransparency=0.4,TextStrokeColor3=Color3.new(0,0,0),
                Visible=ESPSettings.showName,
            },bb)
            ...
            local distLbl=New("TextLabel",{
                ...
                Text="",TextSize=9,Font=Enum.Font.Gotham,
            ...
            selBox.Color3=ESPSettings.boxColor
            selBox.LineThickness=0.04
            selBox.SurfaceTransparency=0.92
            selBox.SurfaceColor3=ESPSettings.boxColor
            selBox.Adornee=char
            selBox.Visible=ESPSettings.showBox
            selBox.Parent=workspace
            data.selBox=selBox

            -- Glow (highlight)
            local hl=Instance.new("SelectionHighlight")
            pcall(function()
                hl=Instance.new("Highlight")
                hl.FillColor=ESPSettings.glowColor
                hl.OutlineColor=ESPSettings.glowColor
                hl.FillTransparency=0.82
                hl.OutlineTransparency=0.3
                hl.Adornee=char
                hl.Enabled=ESPSettings.showGlow
                hl.Parent=workspace
                data.hl=hl
            end)
            pcall(function()
    ...
    local c=pl.CharacterAdded:Connect(DoCreate)
    if ESPData[pl] then ESPData[pl].charConn=c
    else ESPData[pl]={charConn=c} end
end
        ...
        pcall(function()
            ...
            for pl,data in pairs(ESPData) do
                if pl and pl.Character then
                    ...
                    if hrp and data.distLbl then
                        local dist=myHRP and math.floor((myHRP.Position-hrp.Position).Magnitude) or 0
                        pcall(function() data.distLbl.Text=dist.."m" end)
                        local visible=(dist<=ESPSettings.maxDist)
                        if data.nameLbl then pcall(function() data.nameLbl.Visible=ESPSettings.showName and visible end) end
                        if data.distLbl then pcall(function() data.distLbl.Visible=ESPSettings.showName and visible end) end
                        if data.selBox then pcall(function() data.selBox.Visible=ESPSettings.showBox and visible end) end
                        if data.hl then pcall(function() data.hl.Enabled=ESPSettings.showGlow and visible end) end
                        -- update colors live
                        if data.nameLbl then pcall(function() data.nameLbl.TextColor3=ESPSettings.nameColor end) end
                        if data.selBox then
                            pcall(function()
                                data.selBox.Color3=ESPSettings.boxColor
                                data.selBox.SurfaceColor3=ESPSettings.boxColor
                            end)
                        end
                        if data.hl then
                            pcall(function()
                                data.hl.FillColor=ESPSettings.glowColor
                                data.hl.OutlineColor=ESPSettings.glowColor
                            end)
                        end
                    end
                end
...
local function StopESP()
    ...
    for pl,_ in pairs(ESPData) do RemoveESPFor(pl) end
    ESPData={}
end

-- ESP param panel builder
local function BuildESPPanel(inner)
    local ll=Instance.new("UIListLayout");ll.Padding=UDim.new(0,6);ll.Parent=inner

    -- show name toggle
    MakeToggleRow(inner,"Показывать имя",ESPSettings.showName,function(v)
    ...
    MakeToggleRow(inner,"Показывать бокс",ESPSettings.showBox,function(v)
        ...
        for _,data in pairs(ESPData) do
            if data.selBox then pcall(function()data.selBox.Visible=v end) end
        end
    end)

    -- show glow toggle
    MakeToggleRow(inner,"Показывать свечение",ESPSettings.showGlow,function(v)
        ESPSettings.showGlow=v
        for _,data in pairs(ESPData) do
            if data.hl then pcall(function()data.hl.Enabled=v end) end
        end
    end)

    -- max distance slider
    MakeSlider(inner,{
        label="Макс. дистанция",min=50,max=1000,step=25,val=ESPSettings.maxDist,
...
local VISUAL_SECTIONS={
    {title="ВИЗУАЛ",items={
        -- ESP с полными настройками
        {
            label="ESP Players",icon="ESP",
            desc="Имена + боксы + свечение игроков",
            state=false,
            paramH=142,  -- высота панели настроек
            buildParamPanel=BuildESPPanel,
            enable=function() StartESP() end,
            disable=function() StopESP() end,
        },
...
local Open=false  -- starts hidden until preloader done
local CurTab=1
local TAB_DATA={
...
-- ══════════════════════════════════════════════
-- HEADER BUTTONS — абсолютно по правому краю
-- используем Frame-контейнер чтобы кнопки не уплывали
-- ══════════════════════════════════════════════
local btnContainer=New("Frame",{
    Size=UDim2.new(0,68,0,HDR_H),  -- фиксированная ширина для двух кнопок
    Position=UDim2.new(1,-72,0,0), -- прижат к правому краю хедера
...
local minBtn=New("TextButton",{
    ...
    TextColor3=C.TxM,AutoButtonColor=false,ZIndex=8,
},btnContainer)
...
-- INIT — сначала preloader, потом показываем меню
BuildTabs()
RefreshContent()
UpdateBadge()
...
ShowPreloader(function()
    Open=true
    panel.Visible=true
    Notify("LUXURY HUB v27","OK! "..#ALL_ITEMS.." функций · ESP улучшен",C.Acc)
end)

print("LUXURY HUB v27 - OK | "..#ALL_ITEMS.." items")