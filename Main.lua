-- LUXURY HUB v14 FIXED - No += operators
...
local function dS(d,x,y,w,h,col)       d.Position=Vector2.new(x,y);d.Size=Vector2.new(w,h);d.Color=col;d.Visible=true end
local function dL(d,x1,y1,x2,y2,col,th) d.From=Vector2.new(x1,y1);d.To=Vector2.new(x2,y2);d.Color=col;d.Thickness=th or 1;d.Visible=true end
local function dC(d,x,y,r,col,f,th)    d.Position=Vector2.new(x,y);d.Radius=r;d.Color=col;d.Filled=f~=false;d.Thickness=th or 1;d.Visible=true end
local function dT(d,x,y,t,col,sz,ctr)  d.Position=Vector2.new(x,y);d.Text=tostring(t);d.Color=col;d.Size=sz or 14;d.Center=ctr or false;d.Visible=true end
...
local function MRR(border)
    local o={
        ...
        m1=NS(),m2=NS(),m3=NS(),m4=NS()
    }
    if border then
        o.b1=NL();o.b2=NL();o.b3=NL();o.b4=NL()
        o.b5=NC();o.b6=NC();o.b7=NC();o.b8=NC()
    end
    return o
end
...
local function RR(o,x,y,w,h,col,msk,r,bdc,bth)
    ...
    if o.b1 then
        if bdc then
            ...
            o.b5.Visible=false;o.b6.Visible=false
            o.b7.Visible=false;o.b8.Visible=false
        end
    end
end

local function HRR(o)
    for _,k in ipairs({"f","ft","c1","c2","c3","c4","m1","m2","m3","m4",
                        "b1","b2","b3","b4","b5","b6","b7","b8"}) do
...
local function DrawIcon(IL,IC,idx,cx,cy,col)
    ...
    elseif idx==4 then
        ...
        for t=0,7 do
            ...
            dL(IL[t+1],
                cx+math.cos(a)*7.5, cy+math.sin(a)*7.5,
...
-- ══════════════════════════════════════════════
local UI={
    pan=MRR(true), hSep=NL(), hAcc=NL(),
    logoR=MRR(false), logoT=NT(),
...
for i=1,4 do
    SB.items[i]={rr=MRR(true),IL=MIL(10),IC=MIC(3)}
end

local CARDS={}
for i=1,9 do
    CARDS[i]={
        rr=MRR(true), bar=NS(), dot=NC(),
        lbl=NT(), dsc=NT(),
        tR=MRR(false), tKnob=NC(), tGlow=NC(),
    }
end
...
local function DrawToggle(co,x,y,on,msk)
    local W,H=50,26; local r=H/2
    local col=on and C.TgOn or C.TgOff
    RR(co.tR,x,y,W,H,col,msk,r)
    local kx=on and (x+W-r-1) or (x+r+1)
    dC(co.tGlow,kx,y+r,r-1,on and C.AccSoft or C.Black,true)
    dC(co.tKnob,kx,y+r,r-3,C.TgKnob,true)
end

local function HideCard(co)
    HRR(co.rr);co.bar.Visible=false;co.dot.Visible=false
    co.lbl.Visible=false;co.dsc.Visible=false
    HRR(co.tR);co.tKnob.Visible=false;co.tGlow.Visible=false
end

local function HideSettings()
    for _,d in ipairs({SET.ti,SET.l1,SET.l2,SET.l3,SET.l4,
                        SET.slP,SET.div,SET.hkT}) do d.Visible=false end
...
local function Render()
    ...
    if not Open then
        ...
        RR(MINI.rr,mx,my,MBW,MBH,C.Side,C.Base,18,C.AccDim,1.5)
        RR(MINI.iR,mx+MBW/2-13,my+MBH/2-13,26,26,C.Acc,C.Side,8)
        dT(MINI.tx,mx+MBW/2,my+MBH/2-9,"L",C.White,14,true)
        return
    end

    -- SIDEBAR
    local nx,ny=GNP()
    RR(SB.rr,nx,ny,SW,SNAV_H,C.Side,C.Base,22,C.AccDim,1.5)
    for i=1,4 do
        local si=SB.items[i]
        local bx,by=GNI(i)
        local act=CurTab==i
        ...
        RR(si.rr,bx,by,SNAV_SZ,SNAV_SZ,bgC,C.Side,14,bdC,1.5)
        DrawIcon(si.IL,si.IC,i,bx+SNAV_SZ/2,by+SNAV_SZ/2,act and C.Acc or C.TxLow)
    end

    -- PANEL
    RR(UI.pan,PX,PY,PW,PH,C.Panel,C.Base,20,C.Border,1.5)
    dL(UI.hAcc,PX+20,PY+HDR-1,PX+PW-20,PY+HDR-1,C.AccLine,1)
    dL(UI.hSep,PX+16,PY+HDR,  PX+PW-16,PY+HDR,  C.Sep,1)

    -- Header
    RR(UI.logoR,PX+14,PY+14,32,32,C.AccDim,C.Panel,10,C.AccLine,1.5)
    dT(UI.logoT,PX+30,PY+20,"L",C.White,16,true)
    dT(UI.hT1, PX+55,PY+12,"LUXURY",C.TxHi,17)
    dT(UI.hT2, PX+55+80,PY+12,"HUB",C.Acc,17)
    dT(UI.hSub,PX+56,PY+32,"PREMIUM CONTROLS",C.TxLow,8)

    -- Close / Min
    local BS=28; local cX=PX+PW-BS-12; local bY=PY+15; local nX2=cX-BS-7
    local cH=Hov(cX,bY,BS,BS); local nH=Hov(nX2,bY,BS,BS)
    RR(UI.clR,cX,bY,BS,BS,cH and C.Red or C.Surface,C.Panel,9)
    dL(UI.clX1,cX+9,bY+9,cX+BS-9,bY+BS-9,cH and C.White or C.TxMid,1.8)
    dL(UI.clX2,cX+BS-9,bY+9,cX+9,bY+BS-9,cH and C.White or C.TxMid,1.8)
    RR(UI.mnR,nX2,bY,BS,BS,nH and C.CardHov or C.Surface,C.Panel,9)
    dL(UI.mnL,nX2+8,bY+BS/2,nX2+BS-8,bY+BS/2,C.TxMid,2)
    ...
    if CurTab==4 then
        ...
        dT(SET.ti,px,py,"Appearance",C.TxHi,16); py=py+34
        dT(SET.l1,px,py,"REOPEN SIDE",C.TxLow,10); py=py+22
        ...
        for si,side in ipairs({"left","right"}) do
            ...
            RR(SET.sb[si].rr,ox,py,oW,40,
                sel and C.CardOn or (h2 and C.CardHov or C.Card),
                C.Panel,12,sel and C.Acc or nil,1)
            dT(SET.sb[si].tx,ox+oW/2,py+13,
               side:sub(1,1):upper()..side:sub(2),
               sel and C.TxHi or C.TxMid,13,true)
        end
        py=py+40+28

        dT(SET.l2,px,py,"VERTICAL POSITION",C.TxLow,10)
        dT(SET.slP,CX+CW-4,py,math.floor(MiniYPct*100).."%",C.Acc,11,true)
        py=py+22
        local slW=CW-8
        RR(SET.slTR,px,py,slW,8,C.Surface,C.Panel,4)
        local fw=math.max(8,math.floor(MiniYPct*slW))
        RR(SET.slFI,px,py,fw,8,C.Acc,C.Panel,4)
        local hx=px+fw
        ...
        dL(SET.div,CX,py,CX+CW,py,C.Sep,1); py=py+22
        dT(SET.l3,px,py,"HOTKEY",C.TxLow,10); py=py+22
        RR(SET.hkR,px,py,100,34,C.Surface,C.Panel,10,C.AccDim,1)
        dT(SET.hkT,px+50,py+11,"R-SHIFT",C.Acc,12,true)
        py=py+34+28

        dT(SET.l4,px,py,"ACTIVITY",C.TxLow,10); py=py+22
        for ti=1,3 do
            local cnt=0
            for _,it in ipairs(TABS[ti].items) do if it.state then cnt=cnt+1 end end
            local ar=SET.act[ti]; local bW=CW-110
            dT(ar.nm,px,py-1,TABS[ti].name,C.TxMid,12)
            RR(ar.tr,px+65,py+1,bW,10,C.Surface,C.Panel,5)
            local bw=math.floor(cnt/#TABS[ti].items*bW)
            if bw>3 then RR(ar.fi,px+65,py+1,bw,10,C.Acc,C.Panel,5) else HRR(ar.fi) end
            dT(ar.ct,CX+CW-4,py-1,cnt.."/"..#TABS[ti].items,C.TxLow,11,true)
            py=py+22
        end
    else
        HideSettings()
        ...
        for i=1,9 do
            ...
            if not item then
                ...
                RR(co.rr,ix,iy,iw,ROW_H,bgC,C.Panel,14,bdC,1)
                if on then dS(co.bar,ix+10,iy+ROW_H/2-11,3,22,C.Acc);co.bar.Visible=true
                else co.bar.Visible=false end
                dC(co.dot,ix+26,iy+ROW_H/2,5,on and C.Acc or C.TxFaint,true)
                dT(co.lbl,ix+40,iy+ROW_H/2-11,item.label,on and C.TxHi or C.TxMid,14)
                dT(co.dsc,ix+41,iy+ROW_H/2+4, item.desc, on and C.TxMid or C.TxFaint,9)
                DrawToggle(co,ix+iw-50-12,iy+ROW_H/2-13,on,C.Panel)
            end
        end
    end
    ...
    -- Footer
    local FY=PY+PH-FTR
    dL(UI.fSep,PX+14,FY,PX+PW-14,FY,C.Sep,1)
    local cnt=0
    if CurTab<4 then
    ...
    dT(UI.fT2,PX+PW/2,FY+7,tn[CurTab],C.TxMid,9,true)
    dT(UI.fT3,PX+PW-14,FY+7,"RShift",C.TxFaint,9,true)
end
