...
-- ══════════════════════════════════════════════
-- HTTP HELPER — работает во всех экзплойтах
-- ══════════════════════════════════════════════
local function HttpGet(url)
    -- Delta/Solara/Fluxus/Script-Ware используют request()
    if request then
...
local function ShowKeyScreen(onSuccess)
    ...
    local function ValidateKey(key)
        ...
        -- Проверка формата: LX- + длина 26
        if not key:match("^LX%-") or #key ~= 26 then
            SetStatus("❌ Неверный формат (len="..#key..")", C.Red)
            return
        end
        ...
        task.spawn(function()
            -- ✅ БЕЗ UrlEncode — вставляем ключ напрямую
            local url = API_URL .. "/validate?key=" .. key
            print("[KEY] URL: " .. url)

            local httpOk, body = HttpGet(url)
            print("[KEY] httpOk=" .. tostring(httpOk) .. " body=" .. tostring(body))

            loading = false
            Tw(confirmBtn, {BackgroundColor3=C.BG4}, 0.1)

            if not httpOk then
                SetStatus("❌ " .. tostring(body), C.Red)
                return
            end
            ...
            if not ok then
                SetStatus("❌ Ошибка парсинга ответа", C.Red)
                return
            end
...
local function MakeCard(parent,item)
    ...
    local card=New("Frame",{Size=UDim2.new(1,0,0,BASE_H),
        ...
        BorderSizePixel=0,ClipsDescendants=false,ZIndex=1},wrap)
    Rnd(card,12)
    ...
    local hit=New("TextButton",{Size=UDim2.new(1,0,0,BASE_H),
...
local ESPData={}
local ESPConn,ESPUpdateConn=nil,nil
...
local function CreateESPFor(pl)
    ...
    local function DoCreate(char)
        ...
        pcall(function()
            ...
            bbBox.AlwaysOnTop=true;bbBox.LightInfluence=0;bbBox.Parent=hrp
            local function ML(sz,pos)
            ...
            data.boxTop=ML(UDim2.new(1,0,0,2),UDim2.new(0,0,0,0))
            data.boxBot=ML(UDim2.new(1,0,0,2),UDim2.new(0,0,1,-2))
            data.boxL=ML(UDim2.new(0,2,1,0),UDim2.new(0,0,0,0))
            data.boxR=ML(UDim2.new(0,2,1,0),UDim2.new(1,-2,0,0))
    ...
    ESPUpdateConn=RS.Heartbeat:Connect(function()
        pcall(function()
            ...
            for pl,data in pairs(ESPData) do
                ...
                local vis=(dist<=ESPSettings.maxDist)
                if data.nameLbl then pcall(function()
                    data.nameLbl.Visible=ESPSettings.showName and vis
                    data.nameLbl.TextColor3=ESPSettings.nameColor end) end
                if data.distLbl then pcall(function()
                    data.distLbl.Text=dist.."m"
                    data.distLbl.Visible=ESPSettings.showName and vis end) end
                if data.bbBox then pcall(function()
                    data.bbBox.Visible=ESPSettings.showBox and vis
                ...
                if data.hl then pcall(function()
                    data.hl.Enabled=ESPSettings.showGlow and vis
...
local function BuildESPPanel(inner)
    ...
    MakeToggleRow(inner,"Показывать бокс",ESPSettings.showBox,function(v)
        ...
        for _,d in pairs(ESPData) do if d.bbBox then pcall(function()d.bbBox.Visible=v end) end end end)
    MakeToggleRow(inner,"Свечение",ESPSettings.showGlow,function(v)
        ESPSettings.showGlow=v
        for _,d in pairs(ESPData) do if d.hl then pcall(function()d.hl.Enabled=v end) end end end)
        ...
        {label="Server Hop",icon="HOP",desc="Другой сервер",state=false,
         enable=function(it) it.state=false
             task.spawn(function()pcall(function()
                 ...
                 if ok2 then
                     local ok3,data=pcall(function()return game:GetService("HttpService"):JSONDecode(body2)end)
