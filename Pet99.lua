...
pcall(function() if gethui then local gh=gethui(); if gh then GuiRoot=gh end end end)
...
local function SafeClip(str)
    local ok=false
    if not ok then pcall(function() if setclipboard then setclipboard(str);ok=true end end) end
    if not ok then pcall(function() if toclipboard then toclipboard(str);ok=true end end) end
    if not ok then pcall(function() if rbxassetid then end end) end
end

-- Безопасный fireclickdetector
local function SafeFireClick(cd)
...
local function ShowPreloader(onDone)
    ...
    if not pFrame then if onDone then onDone() end return end
...
local function ShowKeyScreen(onSuccess)
    ...
    if not overlay then if onSuccess then onSuccess() end return end
    local box=New("Frame",{
    ...
    local loading=false
    local function SetStatus(txt,col2)
        pcall(function() if statusLbl and statusLbl.Parent then
            statusLbl.Text=txt;statusLbl.TextColor3=col2 or C.TxM end end)
    end
    local function ValidateKey(key)
        ...
        task.spawn(function()
            ...
            if not ok2 then SetStatus("❌ Ошибка парсинга",C.Red);return end
    ...
    if confirmBtn then
        confirmBtn.MouseButton1Click:Connect(function()
            if input then ValidateKey(input.Text) end
        end)
    end
    if input then
...
local function MakeSlider(parent,opts)
    ...
    local knobInner=New("Frame",{Size=UDim2.new(0,8,0,8),Position=UDim2.new(0.5,-4,0.5,-4),
        BackgroundColor3=C.TxW,BorderSizePixel=0,ZIndex=7},knob)
    if knobInner then Rnd(knobInner,4) end
    local dragging=false
    local function SetVal(v)
        v=math.max(mn,math.min(mx,math.floor(v/st+0.5)*st));val=v
        local p2=math.max(0,math.min(1,(val-mn)/(mx-mn)))
        pcall(function() if fill and fill.Parent then fill.Size=UDim2.new(p2,0,1,0) end end)
        pcall(function() if valLbl and valLbl.Parent then valLbl.Text=tostring(fmt(val)) end end)
...
local function MakeToggle(parent,init,onChange)
    ...
    local function Set(s,anim)
        ...
        if anim then
            ...
            pcall(function()if track and track.Parent then track.BackgroundColor3=on and C.Tog or C.BG0 end end)
            pcall(function()if kn and kn.Parent then kn.Position=pos end end)
        end
        pcall(function()
    ...
    if btn then btn.MouseButton1Click:Connect(function()
        Set(not on,true);if onChange then pcall(onChange,on) end end) end
...
local function MakeCard(parent,item)
    ...
    local function ApplyVisual(s)
        pcall(function()
            if card and card.Parent then card.BackgroundColor3=s and C.BG4 or C.BG2 end
            if strk and strk.Parent then strk.Color=s and C.BordA or C.Bord end
            if bar and bar.Parent then bar.Visible=s end
            if lbl and lbl.Parent then lbl.TextColor3=s and C.TxW or C.TxH end
            if dsc and dsc.Parent then dsc.TextColor3=s and C.TxM or C.TxL end
            if iconF and iconF.Parent then iconF.BackgroundColor3=s and C.BG5 or C.BG3 end
            if iconLbl and iconLbl.Parent then iconLbl.TextColor3=s and C.TxW or C.TxL end
            if iconF and iconF.Parent then
        ...
        if paramPanel then
            pcall(function()
                if not paramPanel.Parent then return end
                if s then
    ...
    card.MouseEnter:Connect(function()
        if not item.state then
            ...
            pcall(function()if strk and strk.Parent then strk.Color=C.BordH end end) end end)
    card.MouseLeave:Connect(function()
        if not item.state then
            Tw(card,{BackgroundColor3=C.BG2},0.1)
            pcall(function()if strk and strk.Parent then strk.Color=C.Bord end end) end end)
...
local function GetRemote(name)
    ...
    local ok2,rem=pcall(function() return RS2:FindFirstChild(name,true) end)
    if ok2 then return rem end
    return nil
end
...
local function GetPlayerData()
    local data=nil
    pcall(function()
        data=LP:FindFirstChild("Data") or
...
local function GetCoins()
    ...
    if data then
        local coins=nil
        pcall(function()
            coins=data:FindFirstChild("Coins") or
    ...
    {title="🥚 ФАРМ ЯИЦ",items={
             ...
             it.conn=RS.Heartbeat:Connect(function()
                 pcall(function()
                     ...
                     for _,egg in ipairs(eggs) do
                         local ok3,d=pcall(function()return (hrp.Position-egg.part.Position).Magnitude end)
                         if ok3 and d<nearDist then nearDist=d;nearest=egg end
                     end
                     if nearest then
                         if nearDist>5 then
                             pcall(function() hrp.CFrame=CFrame.new(nearest.part.Position+Vector3.new(0,3,0)) end)
                         end
                         ...
                         pcall(function()
                             ...
                             if clickDetector then SafeFireClick(clickDetector) end
                         end)
                     end
                 end)
             end)
             ...
             it.conn=RS.Heartbeat:Connect(function()
                 pcall(function()
                     ...
                     for _,item2 in ipairs(items) do
                         local ok3,d=pcall(function()return (hrp.Position-item2.pos).Magnitude end)
                         if ok3 and d<nearDist then nearDist=d;nearest=item2 end
                     end
                     if nearest and nearDist>3 then
                         pcall(function() hrp.CFrame=CFrame.new(nearest.pos+Vector3.new(0,2,0)) end)
                     end
                 end)
             end)
         end,
         ...
         enable=function(it)
             it.conn=RS.Heartbeat:Connect(function()
                 pcall(function()
                     ...
                     for _,item2 in ipairs(items) do
                         pcall(function()
                             if (hrp.Position-item2.pos).Magnitude<30 then
                                 local cd=nil
                                 if item2.obj:IsA("Model") then
                                     cd=item2.obj:FindFirstChildOfClass("ClickDetector")
                                 elseif item2.obj:IsA("BasePart") then
                                     cd=item2.obj:FindFirstChildOfClass("ClickDetector")
                                 end
                                 if cd then SafeFireClick(cd) end
                                 FireServer("BreakObject",item2.obj)
                                 FireServer("Break",item2.obj)
                                 FireServer("BreakPiggyBank",item2.obj)
                             end
                         end)
                     end
                 end)
        ...
        {label="Coin Magnet",icon="🧲",desc="Притягивать монеты к себе",state=false,conn=nil,_range=50,
         param={label="Радиус магнита",min=10,max=200,step=10,val=50,
          ...
          apply=function(v,it)if it then it._range=v end end},
         enable=function(it)
                     ...
                     if #zones>0 then
                         ...
                         pcall(function()
                             if zone:IsA("BasePart") then pos=zone.Position
                                 ...
                                 local bp=zone:FindFirstChildOfClass("BasePart")
                                 if bp then pos=bp.Position end
                             end
                         end)
                         if pos then pcall(function() hrp.CFrame=CFrame.new(pos+Vector3.new(0,5,0)) end) end
                     end
                 end
    ...
    {title="🚀 ДВИЖЕНИЕ",items={
        ...
        {label="Speed Boost",icon="💨",desc="Ускорение персонажа",state=false,
         ...
         enable=function(it)pcall(function()
             local h=LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
             if h then h.WalkSpeed=16*(it.param and it.param.val or 3) end end)end,
        ...
        {label="Fly",icon="🛸",desc="Полёт WASD+Space/Ctrl",state=false,conn=nil,_bv=nil,_bg=nil,_spd=80,
         ...
         enable=function(it)
             it._spd=it.param and it.param.val or 80
             pcall(function()
         ...
         enable=function(it)
             it.conn=RS.Stepped:Connect(function()pcall(function()
                 if LP.Character then for _,p in ipairs(LP.Character:GetDescendants()) do
                     pcall(function() if p:IsA("BasePart") then p.CanCollide=false end end) end end end) end)
         end,
         disable=function(it)
             if it.conn then it.conn:Disconnect();it.conn=nil end
             pcall(function()if LP.Character then for _,p in ipairs(LP.Character:GetDescendants()) do
                 pcall(function() if p:IsA("BasePart") then p.CanCollide=true end end) end end end)
         end},
    ...
    {title="📊 HUD / СТАТЫ",items={
        ...
        {label="Coins HUD",icon="🪙",desc="Монеты + монеты/сек",state=false,_gui=nil,conn=nil,
         enable=function(it)pcall(function()
             ...
             it._gui=fr
             it.conn=RS.Heartbeat:Connect(function()pcall(function()
                 if not coinsLbl or not coinsLbl.Parent then return end
                 local c=GetCoins()
        ...
        {label="Stats HUD",icon="📊",desc="Полная статистика игрока",state=false,_gui=nil,conn=nil,
         enable=function(it)pcall(function()
             ...
             it._gui=fr
             it.conn=RS.Heartbeat:Connect(function()pcall(function()
                 if not statsLbl or not statsLbl.Parent then return end
                 local data=GetPlayerData()
                 local txt=""
                 if data then
...
local function UpdateBadge()
    ...
    pcall(function()
        if activeBadge and activeBadge.Parent then
...
local function RefreshContent()
    ...
    pcall(function()
        if fTabName and fTabName.Parent then fTabName.Text=td.name end
    end)
...
local function BuildTabs()
    ...
    for i,td in ipairs(TAB_DATA) do
        ...
        pill.MouseEnter:Connect(function()
            if CurTab~=iCopy then Tw(pill,{BackgroundColor3=C.BG3},0.1) end end)
        pill.MouseLeave:Connect(function()
            if CurTab~=iCopy then Tw(pill,{BackgroundColor3=C.BG2},0.1) end end)
        pill.MouseButton1Click:Connect(function()
            CurTab=iCopy;BuildTabs();RefreshContent()
        end)
