...
-- Специальный админ-ключ (измените на свой)
local ADMIN_KEY = "NIOMERO"

-- Функция проверки админ-ключа
local function isAdminKey(key)
    return key == ADMIN_KEY
end

if isfile("IceWare/Key System/Key.json") then
    local savedKey = game:GetService("HttpService"):JSONDecode(readfile("IceWare/Key System/Key.json")).Key
    
    -- Проверка на админ-ключ
    if isAdminKey(savedKey) then
...
do
    ...
    Section:Button({
        Name = "Check Key",
        Callback = function()
            local inputKey = Options.KeyInput.Value
            
            -- Проверка на админ-ключ
            if isAdminKey(inputKey) then
                Library:Notification({
                    Name = "Admin Access",
                    Description = "Admin key detected! Loading script...",
                    Duration = 3
                })
                if not isfolder("IceWare/Key System") then
                    makefolder("IceWare/Key System")
                end
                writefile("IceWare/Key System/Key.json", game:GetService("HttpService"):JSONEncode({Key = inputKey}))
            ...
            if (status.code == "KEY_VALID") then
                ...
                writefile("IceWare/Key System/Key.json", game:GetService("HttpService"):JSONEncode({Key = inputKey}))
                getgenv().script_key = tostring(inputKey)
                api.load_script()
            else
                Library:Notification({
                    Name = "Notification",
